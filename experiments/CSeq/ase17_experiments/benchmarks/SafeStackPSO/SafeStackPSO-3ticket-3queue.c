...
void __CS_atomic_Write_atomic(__CS_typeV codeVar, intV val){
   ...
   __CS_typeTicket  tick1,tick2;
	...
	ticket_out_thread[__cs_thread_index] = (ticket_pos<=ticket_out_thread[__cs_thread_index])?ticket_out_thread[__cs_thread_index]:ticket_pos;
    ticket_out_thread_of[codeVar][__cs_thread_index] = ticket_pos;
...
void __CS_atomic_Write(__CS_typeV codeVar, intV val){
   ...
   __CS_typeTicket  tmp, newTicket,tick1,tick2;
    ...
    __CPROVER_assume (1
		      ...
		      & (newTicket <= MaxTicket)
			  //& (newTicket >= ticket_out_thread[__cs_thread_index])
			  & (newTicket >= ticket_out_thread_of[codeVar][__cs_thread_index]) //PSO
		      & (tmp >= ticket_pos)//all tickets must be different
			  & (ticket1[codeVar] <= tmp)
					//& (tmp <= MaxTicket)//all tickets must be different
			  &(
    ...
    ticket_pos = tmp;
    __last_thread_value_of[codeVar][__cs_thread_index] = val;
	ticket_out_thread[__cs_thread_index] = (newTicket<=ticket_out_thread[__cs_thread_index])?ticket_out_thread[__cs_thread_index]:newTicket;
    ticket_out_thread_of[codeVar][__cs_thread_index] = newTicket;
...
void __CS_atomic_Fence_and_Write(__CS_typeV codeVar, intV val){
   ...
   __CS_typeTicket  tmp, newTicket,tick1,tick2;
    ...
    __CPROVER_assume (1
              ...
              //& (newTicket == tmp)   //per SC
              & (newTicket <= MaxTicket)
              & (newTicket >= ticket_out_thread[__cs_thread_index]) // TRUC: a fence has been called already
              // & (newTicket >= ticket_out_thread_of[codeVar][__cs_thread_index]) //PSO
              & (tmp >= ticket_pos)//all tickets must be different
              & (ticket1[codeVar] <= tmp)
                    //& (tmp <= MaxTicket)//all tickets must be different
    ...
    ticket_pos = tmp;
    __last_thread_value_of[codeVar][__cs_thread_index] = val;
    ticket_out_thread[__cs_thread_index] = newTicket; // TRUC
    ticket_out_thread_of[codeVar][__cs_thread_index] = newTicket;
    //ticket_out_thread[__cs_thread_index] = ticket_out_thread_of[codeVar][__cs_thread_index] = newTicket;
