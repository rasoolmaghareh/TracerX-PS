...
def step3GenerateConfigFileEX(env, out, configFilename, logger=None):
    ...
    if env['strategy'] == 'quarter':
        ...
        print "strategy 3: window size is set manually with window size %s and pick %s" % (
            env['window-length'], env['picked-window'])
...
def callTranslatorSwarmPercentage(env, inputfile, logger=None):
    ...
    if not result:
        ...
        if env['configFile'] == '':
            print "[Swarm by percentage settings] Automatic creating configuration file..."
...
def callTranslatorSwarmWMM(env, inputfile, logger=None):
    ...
    if not result:
        ...
        # Generating configuration file
        if env['configFile'] == '':
            print "[Swarm for WMM] Automatic creating configuration file..."
            if not env['automatic']:
                print 'Please set -A option if you want to automatically generate instances'
                sys.exit(2)
            env['configFile'] = inputfile[:-2] + '_auto_config%s.json' % env['suffix']
...
def callTranslatorSwarm(env, inputfile, logger=None):
    # setting up cmd
    cmdline = step1SetupCMD(env, inputfile, isSwarm=True, logger=logger)
    # get thread lines
    out, result = step2GetThreadLines(env, cmdline, inputfile)
    if not result:
        ...
        # Generating configuration file
        if env['configFile'] == '':
            print "[Swarm by normal settings] Automatic creating configuration file..."
            if not env['automatic']:
                print 'Please set -A option if you want to automatically generate instances'
                sys.exit(2)
            env['configFile'] = inputfile[:-2] + '_auto_config%s.json' % env['suffix']
