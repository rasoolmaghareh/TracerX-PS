...
def backendWorker(cmdline):
    ...
    return (inputfile, outcome, backendtime, memory)


def callBackendSwarm(env, seqfileList, realstarttime, logger=None):
    format = env['backend']
    # Setting up CMD
    cmdline = step1SetUpCMD(env)
    if env['execution-mode'] == 'sequential':
        ...
        while initalTimeout <= env['timelimit']:   # Sequential loop until reach time limit
            if foundBug or allTrue:     # Break when found bugs or all instance are true
                break
            print 'Analyzing %s instance(s) SEQUENTIALLY with %ss timeout for each instance' % (
                noInstance, initalTimeout)
            print '============================================================================'
            sys.stdout.flush()
        ...
        while initalTimeout <= env['timelimit']:
            ...
            pool_size = env['cores']
            print 'Analyzing %s instance(s) IN PARALLEL with %ss timeout for %s processes' % (
                noInstance, initalTimeout, pool_size)
            print '============================================================================'
            sys.stdout.flush()
