...
def generateCombinationDoubleWindowsRandomize(seq, r, limit, consecutive=True):
    ...
    while True:
        if time.time() > timeout or i >= loopbound:
            break
        samples = random.sample(xrange(len(seq)-1), r)
...
def generateCombinationDoubleWindows(seq, r, limit, consecutive=True):
    ...
    while True:
        if time.time() > timeout or i >= loopbound:
            break
        for samples in itertools.combinations(xrange(len(seq)-1), r):
...
def generateCombinations(seq, r, total, consecutive=True, limit=0, randomness=True):
    ...
    if limit > 0:
        if randomness:
            sample = list2dict(random.sample(xrange(total), limit))
        else:
            sample = list2dict(xrange(limit))
    if consecutive:
        i = 0
        count = 0
...
class ConfigGenerator():
    ...
    def __init__(self, data, windowWidth, pickWindow, instanceslimit,
    ...
    def generatingConfig(self, outputfile, softLimit=0, hardLimit=0,
        ...
        for thread in self.data:
            ...
            if thread not in self.skiplist and self.windowWidth < max_contextswitch - 2 and self.realWindowWidth*self.pickWindow < max_contextswitch:
                i = 1
                while i < max_contextswitch:
                    lrange = i
                    rrange = i + self.windowWidth - 1
        ...
        if softlimitedInstances > chosenInstances:
            partial = True
            if randomness:
                sampleList = random.sample(xrange(softlimitedInstances), chosenInstances)
...
class ConfigGeneratorWMM():
    ...
    def generatingConfigWMM(self, outputfile, softLimit=0, hardLimit=0, verbose=False):
        ...
        numInstance = 0
        for item in itertools.product(*threadInstances):
            if not randomness:     # Generating all possible instances
                key = 's%s' % index
                self.generatedData[key] = {}   # Each sample
                for i in range(0, len(item)):
                    self.generatedData[key][threadNames[i]] = item[i]
