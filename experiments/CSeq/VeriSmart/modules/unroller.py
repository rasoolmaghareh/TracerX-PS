...
"""
VERSION = 'unroller-0.9-2016.11.20'   # Overhaul
...
Changelog:
    2017.09.04  fix for extracting line number from node coord (from pycparser 2.18+)
...
class unroller(core.module.Translator):
    ...
    def visit_For(self, n):
        ...
        if self.softunwindbound:
            ''' When softunwindbound, definite loops are fully unwound unless there is bounded on those loops
            '''
            if (self._calculateLoopBound(n) is not None):
        ...
        for i in range(0, bound):
            ...
            if n.next:
                linenumber = core.utils.removeColumnFromCoord(n.next.coord)
