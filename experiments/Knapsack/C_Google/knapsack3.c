...
// #define N 2000 // seems to be the limit ...
#define N 46
...
// int item_weight[50] = {90, 75, 75, 70, 115,
//        35, 85, 55, 65, 125,
//        125, 95, 90, 105, 135,
//        45, 110, 95, 115, 145,
//        60, 105, 80, 75, 155,
...
struct half_interpolant copy(struct half_interpolant a);
void print(struct half_interpolant a);
...
void init() {
  srand(12345);
  for (int i = 1; i <= N; i++) {
    item_weight[i] = 5 * (1 + rand() % (MAXRAND1 + 1));
    // item_weight[i] = 10 * i;
    // item_profit[i] = 100 * (N - i + 1);
  }

  for (int i = 1; i <= N; i++) {
    printf("Item %2d: Weight = %ld\n", i, item_weight[i]);
  }
...
struct half_interpolant decide(int level) {
  ...
  for (int k = 1; k <= interpolant_table_count; k++) {
    if (interpolant_table[k].node == level) {
      DEBUG_PRINT("\t****Subsumption check: ");
      print(interpolant_table[k]);

      int flag = 0;
      for (int s = 0; s < interpolant_table[k].slack_len; s++) {
      ...
      if (flag == 0) {
        ...
        DEBUG_PRINT("\t*** Subsumed1: level %d weight  %ld, int:", level,
                    ss.weight);
        struct half_interpolant s = copy(h);
        print(s);
        return s;
      }
    }
    ...
    for (int s = 0; s < h.slack_len; s++) {
      if (j)
        if (h.slack_weight[s] - item_weight[level] > 0)
          h.slack_weight[s] = h.slack_weight[s] - item_weight[level];
        else
          h.slack_weight[s] = 0;
    }

    half[j] = copy(h);
    DEBUG_PRINT("\t Printing Half[%d]: ", j);
    print(half[j]);
  }
  ...
  print(full);
  interpolant_table[++interpolant_table_count] = copy(full);
  return full;
}
...
struct half_interpolant copy(struct half_interpolant a) {
  ...
  b.safety = a.safety;
  b.node = a.node;
  b.slack_len = a.slack_len;
  b.slack_weight = (long *)malloc(WEIGHT_BOUND * sizeof(long));
  for (int i = 0; i < a.slack_len; i++) {
    b.slack_weight[i] = a.slack_weight[i];
  }
  return b;
}
...
void print(struct half_interpolant a) {
  DEBUG_PRINT(
      "\t PRINT INTERPOLANT: Node: %d, Safety: %ld: slack_len:  %d, half int: ",
      a.node, a.safety, a.slack_len);
  for (int s = 0; s < a.slack_len; s++)
    DEBUG_PRINT(" %ld, ", a.slack_weight[s]);
  DEBUG_PRINT(" \n ");
}
